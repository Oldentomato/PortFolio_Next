...
수식으로는 $$\frac{1}{n}  \sum_ㅤ^ ㅤ ( \hat{y} - y)^{2}$$ 이다.  $\hat{y}$가 실제값이고 y가 예측값이다.  
...
수식으로 표현하면 $$w1' = w1 - \alpha \frac{\partial*E}{\partial * w1}$$
...
$\alpha \frac{\partial*E}{\partial * w1}$ 에서 $\alpha$가 learning rate이다.  
