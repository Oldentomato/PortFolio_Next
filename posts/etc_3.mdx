...
그다음은 intent 등록이다.  
방법은 크게 두가지가 있다. 하나는 manifest xml 코드에서 <receiver>를 이용해 등록하는 것이고,  
...
```
android:name은 해당 파일의 <manifest> 에서 package명에서 sms,mms 수신처리코드가 작성된 클래스명을 써주면 된다고 한다.  
