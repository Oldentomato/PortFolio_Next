...
services:
  reverse-proxy:
    ...
    command:
      ...
      - --api.dashboard=true
      - --api.insecure=true
      # - --ping.entryPoint=fastapi
      - --entrypoints.https.address=:443 # HTTPS 포트
  ...
  fastapi-server:
    ...
    deploy:
      ...
      labels:
        ...
        - traefik.http.routers.fastapi-server.middlewares=redirect-to-https
...
services:
  ...
  grafana: 
    ...
    environment:
      - GF_SECURITY_ADMIN_USER=test      # 기본 관리자 계정명
      - GF_SECURITY_ADMIN_PASSWORD=qwer1234  # 기본 관리자 비밀번호
      - GF_DASHBOARDS_JSON_ENABLED=true   
    volumes:
      - ./grafana/provisioning/datasources:/etc/grafana/provisioning/datasources  # 데이터 소스 프로비저닝
      - ./grafana/provisioning/dashboards:/etc/grafana/provisioning/dashboards    # 대시보드 프로비저닝
