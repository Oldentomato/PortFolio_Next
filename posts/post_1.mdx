...
## 서버 구조  
![img1](https://github.com/Oldentomato/PortFolio_Next/blob/main/postsimg/post_1/img_1.png)  
...
## 개발 환경 구조
### 코드 작성 환경
![img2](https://github.com/Oldentomato/PortFolio_Next/blob/main/postsimg/post_1/img_2.png)  
...
### 시각화 서버 환경
![img2](https://github.com/Oldentomato/PortFolio_Next/blob/main/postsimg/post_1/img_3.png)  
    ...
    networks:
      cloud-socket-proxy:
        aliases:
          - socket-proxy
 ```
 여기서 socket-proxy의 역할은  Docker API에 대한 보안과 접근 제어를 강화이다.  
 Traefik과 같은 서비스가 Docker 엔진과 통신해야 할 때, 직접적으로 Docker 소켓에 접근하는 대신 프록시를 통해 제한된 권한으로 접근할 수 있도록 구성하는 것이다.  
 fastapi 서버 docker-compose.yaml은 다음과 같다.  
 ```yaml
version: '3.8'
...
networks:
 ...
 현재 급하게 구성하느라 아직 미흡한 부분이 많이 때문에 많이 보완해야 한다.  
 SSL인증까지 적용된 서비스가 구축되면 이 구조에 대해 더 자세히 기술할 예정이다.  

[참고](https://snark16x.gitlab.io/16x/traefik-swarm-production/)